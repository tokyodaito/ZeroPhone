...
- **Home Launcher.** `MainActivity` объявлена с `CATEGORY_HOME` + `CATEGORY_DEFAULT` (и `MAIN/LAUNCHER`), при нажатии кнопки «Домой» система предлагает выбрать ZeroPhone домашним экраном. Ниже контекстного блока отображается сетка launchable-приложений **только из allowlist**.
- **ZeroLauncher — главный экран.** Вместо app drawer — только полезная информация: текущее время, следующее событие календаря (с локацией), задачи/напоминания (добавление и выполнение прямо с главного экрана), счётчик важных непрочитанных сообщений, бейдж активного режима и ряд быстрых действий (**Позвонить / Написать / Доехать / Заплатить / Камера**). Каждое быстрое действие — системный intent-шаблон, привязанный к логической capability; видимость/доступность определяет контекстный движок. Нет app drawer, бесконечных страниц иконок, сторонних widgets, рекомендаций, новостей и Google Discover.
...
4. **Домашний экран.** Нажать «Домой», выбрать ZeroPhone в качестве лаунчера (Always). Убедиться, что сверху показаны часы, следующее событие календаря (после выдачи разрешения `READ_CALENDAR`), задачи и режим; ниже — сетка только allowlist-приложений. Быстрые действия запускают набор номера / SMS / карты / камеру (или отключены, если движок ограничил capability). На экране нет app drawer, сторонних widgets и лент.
...
8. **Emergency unlock.** Нажать «Экстренная разблокировка», выбрать длительность (пресеты 5/15/30/60 мин или своя) и подтвердить: все заблокированные приложения становятся доступны, отображается обратный отсчёт до re-lock. Проверить досрочную блокировку кнопкой «Заблокировать сейчас», затем (открыв новое окно) дождаться истечения — блокировка применяется автоматически.

...
- **Контекстный движок.** Каждая возможность (пакет или логическая capability) резолвится движком ровно в одно из пяти состояний: `AVAILABLE` / `RESTRICTED` / `TEMPORARILY_AVAILABLE` / `CONTEXTUAL` / `BLOCKED`. Решения переводятся в `setPackagesSuspended` только для `BLOCKED`; защищённые `SuspendPolicy` пакеты никогда не блокируются. Правила конфликтуют детерминированно (специфичность → приоритет → id); гранты («временно доступна», long-press на быстром действии — 15 минут) истекают автоматически с re-lock через AlarmManager; ограниченные возможности получают дневной бюджет времени (локальный день), который наполняется реальным использованием через UsageStats (Device Owner выдаёт себе доступ автоматически) и показывается остатком на карточке действия. Границы TimeWindow-правил ставят отдельный будильник — «блокировать с 22:00» срабатывает без открытия лаунчера.
...
- **Настройки.** Экран настроек с главного экрана (шестерёнка): дефолтная длительность emergency-окна, редактор правил доступности (цель — способность или приложение; условие — всегда / окно времени с днями недели / режим; решение — разрешить / дневной лимит / запретить) и редактор приоритетных пакетов уведомлений.
- **Задачи.** Добавление, отметка выполнения, редактирование (заголовок + напоминание-время с TimePicker, «сегодня → завтра» при прошедшем времени), удаление, полный список со свёрткой и очисткой выполненных; отсортированы по дедлайну.
...
- Gradle-мульти-модуль (монорепа, растёт по фазам):
  ...
  - `:feature:home`, `:feature:allowlist`, `:feature:settings` — экраны;
...
10. **Настройки.** Открыть шестерёнку на главном экране: сменить дефолтную длительность emergency (применяется к следующему окну), добавить правило «Запретить / Позвонить / По времени 22:00–07:00», проверить, что карточка «Позвонить» гаснет в окне (и без открытия лаунчера — по будильнику границы), включить приоритетное приложение в списке важных уведомлений.
11. **Снятие admin.** `adb shell dpm remove-active-admin com.numenlabs.zerophone/.ZeroDeviceAdminReceiver` — приложение перестаёт блокировать, работает как обычный лаунчер.

...
fun AllowlistScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text("Управление allowlist", style = MaterialTheme.typography.titleLarge)
        }
        OutlinedTextField(
            value = query,
        ...
        if (!deviceOwner) {
            Text(
                text = "Не Device Owner: изменения сохраняются, но блокировка не применяется.",
        ...
        if (visibleApps.isEmpty()) {
            Box(
                ...
                Text(
                    text = if (query.isBlank()) "Нет приложений" else "Ничего не найдено",
